...
describe('linkBuilder', () => {
  ...
  describe('orbitUrlToExplorerLink()', () => {
    it('converts a simple orbit link to a endpoint explorer link', () => {
      expect(linkBuilder.orbitUrlToExplorerLink('http://orbit-testnet.rover.network/ledgers/1173'))
        .to.equal('#explorer?resource=ledgers&endpoint=single&values=eyJsZWRnZXIiOiIxMTczIn0%3D');
    });

    it('converts a templated orbit link to a endpoint explorer link', () => {
      expect(linkBuilder.orbitUrlToExplorerLink('http://orbit-testnet.rover.network/ledgers/1173/transactions{?cursor,limit,order}'))
    ...
    it('converts a non url into undefined', () => {
      expect(linkBuilder.orbitUrlToExplorerLink('laxmicoins')).to.be.a('undefined');
    });
  });
});